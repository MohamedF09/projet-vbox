...
case "$ACTION" in
    ...
    N)
        ...
        # Configuration réseau NAT et priorité au boot PXE (--boot1 net)
        VBoxManage modifyvm "$VM_NAME" --memory "$RAM_MB" --nic1 nat --boot1 net --boot2 disk
        ...
        echo "VM '$VM_NAME' créée avec succès (Boot PXE activé)."
        ;;
    S)
